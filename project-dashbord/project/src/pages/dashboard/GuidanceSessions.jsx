...
import { Search, Filter, Star, Video, MapPin, ChevronDown, X, CheckCircle2, Grid, List, GraduationCap, BookOpen, Target } from 'lucide-react';
...
export default function GuidanceSessions() {
  ...
  const guidanceCounsellors = counsellors.filter(c =>
    c.specialization.includes('Guidance') ||
    c.specialization.includes('Engineering') ||
    c.specialization.includes('Medical') ||
    c.specialization.includes('Commerce')
  );

  const filtered = guidanceCounsellors.filter((c) => {
  ...
  const specializations = ['all', ...new Set(guidanceCounsellors.map((c) => c.specialization))];
  ...
  const categories = [
    { name: 'Personal Mentoring', icon: Target, color: 'primary' },
    { name: 'Study Groups', icon: GraduationCap, color: 'accent' },
    { name: 'Academic Coaching', icon: BookOpen, color: 'success' },
  ];
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-accent via-primary to-secondary p-6 lg:p-8"
      >
        <div className="absolute -top-20 -right-20 w-60 h-60 bg-white/10 rounded-full blur-3xl" />
        <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-accent/20 rounded-full blur-2xl" />
        <div className="relative z-10">
          <h1 className="text-2xl lg:text-3xl font-bold text-white mb-2">Guidance Sessions</h1>
          <p className="text-white/80 max-w-xl">
            Connect with expert mentors who will help you navigate your academic journey.
          </p>
        </div>
      </motion.div>
        ...
        {categories.map((cat, index) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="font-medium text-text text-sm">{cat.name}</h3>
            </motion.div>
          );
        })}
      </div>
          ...
          <div className="flex rounded-xl overflow-hidden border border-border dark:border-dark-border">
            <button
              ...
              className={cn(
                ...
                viewMode === 'grid' ? 'bg-primary text-white' : 'bg-white dark:bg-dark-card text-text-muted'
              )}
            >
            ...
            <button
              ...
              className={cn(
                'px-3 py-2.5 transition-colors',
                viewMode === 'list' ? 'bg-primary text-white' : 'bg-white dark:bg-dark-card text-text-muted'
              )}
            >
      ...
      <AnimatePresence>
        {showFilters && (
          ...
          >
            <div className="bg-white dark:bg-dark-card rounded-xl p-4 border border-border/50">
              <label className="block text-xs font-medium text-text-muted mb-2">Specialization</label>
              <select
      ...
      {/* Results */}
      <p className="text-sm text-text-muted">Showing {filtered.length} mentors</p>
      ...
      <div className={cn(
        ...
        {filtered.map((mentor, index) => (
          <motion.div
            ...
            className={cn(
              'group bg-white dark:bg-dark-card rounded-2xl border border-border/50 hover:border-accent/30 hover:shadow-card-hover transition-all duration-300',
            ...
            {viewMode === 'grid' ? (
              <>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-accent to-primary flex items-center justify-center">
                      <span className="text-white font-semibold text-sm">{mentor.avatar}</span>
                    </div>
                    <div>
                      <h3 className="font-semibold text-text">{mentor.name}</h3>
                      <p className="text-xs text-accent font-medium">{mentor.specialization}</p>
                    </div>
                  </div>
                  <span className={cn(
                    'px-2.5 py-1 rounded-lg text-xs font-medium',
                    mentor.status === 'active' ? 'bg-success/10 text-success' : 'bg-gray-100 text-text-muted'
                  )}>
                    Active
                  </span>
                </div>
                <div className="flex items-center gap-1 mb-4">
                  ...
                  <span className="text-sm font-medium text-text">{mentor.rating}</span>
                  <span className="text-xs text-text-muted ml-1">({mentor.students} students)</span>
                </div>
                <div className="flex items-center justify-between pt-4 border-t border-border/50">
                  <span className="text-xs text-text-muted">{mentor.experience}</span>
                  <button
                    onClick={() => setSelectedCounsellor(mentor)}
                    className="px-4 py-2 rounded-lg bg-gradient-to-r from-accent to-primary text-white text-xs font-semibold hover:opacity-90 transition-opacity"
                  >
                    Book Session
                  </button>
                </div>
              ...
              <>
                ...
                <span className="text-xs text-text-muted hidden md:block">{mentor.experience}</span>
                <button
                  onClick={() => setSelectedCounsellor(mentor)}
                  className="px-4 py-2 rounded-lg bg-gradient-to-r from-accent to-primary text-white text-xs font-semibold hover:opacity-90 transition-opacity"
                >
            ...
            >
              <div className="flex items-center justify-between p-5 border-b border-border/50">
                <h3 className="text-lg font-semibold text-text">
                  {bookingSuccess ? 'Session Booked' : 'Book Guidance Session'}
                </h3>
                <button
                  onClick={() => { setSelectedCounsellor(null); setBookingSuccess(false); }}
                ...
                {bookingSuccess ? (
                  ...
                  <div className="space-y-4">
                        ...
                        {timeSlots.map((slot) => (
                          <button
                            ...
                            className={cn(
                              ...
                              selectedSlot === slot
                                ? 'bg-gradient-to-r from-accent to-primary text-white border-primary'
                    ...
                    <button
                      ...
                      className={cn(
                        'w-full py-3 rounded-xl font-semibold text-sm text-white bg-gradient-to-r from-accent to-primary hover:opacity-90 transition-opacity',
